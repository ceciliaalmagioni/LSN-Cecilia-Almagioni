...
{ 
  ...
  cout << "misuro" << endl;
  int nconf = 1;
  for(int iblk=1; iblk <= nblk; iblk++) //Simulation: per ogni blocco
  {
    Reset(iblk);   //Reset block averages
    for(int istep=1; istep <= nstep; istep++) //per ogni passo nel blocco
    {
      Move();
      Measure();
...
//Prepare arrays for measurements PSONO INTERI, cosa rapppr?
  iv = 0; //Potential energy
  it = 1; //Temperature
  ik = 2; //Kinetic energy
  ie = 3; //Total energy
  ipr = 4; //Total pressure
...
//Read initial configuration
  ...
  {
    ReadConf.open("config.in"); //È LA FCC per 108 part
    ...
    for (int i=0; i<npart; ++i)
    {
      ...
      vz[i] = rnd.Gauss(0.,sqrt(temp)); //genero v con distrib maxwelliana
      sumv[0] += vx[i]; //calcolo v deriva
      sumv[1] += vy[i];
      sumv[2] += vz[i];
    }
    ...
    for (int i=0; i<npart; ++i)
    {
      vx[i] = vx[i] - sumv[0]; //sottraggo v deriva per particella
      vy[i] = vy[i] - sumv[1];
      vz[i] = vz[i] - sumv[2];
      sumv2 += vx[i]*vx[i] + vy[i]*vy[i] + vz[i]*vz[i]; //vquadr
    }
    sumv2 /= (double)npart; //vquadr per part
    fs = sqrt(3 * temp / sumv2);   // fs = velocity scale factor 
    cout << "velocity scale factor: " << fs << endl << endl;
    for (int i=0; i<npart; ++i)
    {
      vx[i] *= fs; //velocità di scala ???
      vy[i] *= fs;
      vz[i] *= fs;
    }
  }
  ...
  for (int i=0; i<npart; ++i)
  {
    ReadConf >> x[i] >> y[i] >> z[i]; //leggo la configurazione in unità di lato + PBC
    x[i] = Pbc( x[i] * box );
    y[i] = Pbc( y[i] * box );
    z[i] = Pbc( z[i] * box );
  }
  ...
  for (int i=0; i<npart; ++i)
  {
    ...
    {
      xold[i] = Pbc(x[i] - vx[i] * delta); //calcolo config vecchie da v
...
//Print initial values for measured properties 
  ...
  cout << "Initial pressure         = " << walker[ipr] << endl; // CHECK
...
{
  double v = 0.0, kin=0.0, pr=0.0, p=0.0;
  double vij;
  double dx, dy, dz, dr; 

  for(int i=0; i < dimg; i++) { g[i] = 0;}
...
//update histogram of g(r)
      ...
      if(dr < rcut) //Potential cutoff
      {
        vij = 1.0/pow(dr,12) - 1.0/pow(dr,6);
        pr = 48.0/pow(dr,12) - 24.0/pow(dr,6);
        v += vij;
        p+=pr;
      ...
      min_dist = box/2.;
      hist_index = dimg * (dr/min_dist);
      
      if (dr < min_dist) g[hist_index] += 2;
      
  //    cout << "prova" << g[hist_index] << endl;
    }          
  }

  for (int i=0; i<npart; ++i) kin += 0.5 * (vx[i]*vx[i] + vy[i]*vy[i] + vz[i]*vz[i]); //Measure kinetic en
...
/*    for (int i=0; i<npart-1; ++i){
    	...
    	for (int j=i+1; j<npart; ++j) {// ciclo interno j=i+1...N
    	
    	dx = Pbc( x[i] - x[j] );  // distance ip-i in pbc
        dy = Pbc( y[i] - y[j] );
        dz = Pbc( z[i] - z[j] );
 ...
 //       cout << pr << endl;
  ...
  vtail = 8.0 * M_PI * rho * (pow(rcut, -9)/9. - pow(rcut, -3)/3.) * npart;
  ptail = 3 * 32. * M_PI * rho * npart * (pow(rcut, -9)/9.-pow(rcut, -3)/6.)/(vol);

  walker[iv] = 4.0 * v + vtail; // Potential energy
  walker[ik] = kin; // Kinetic energy
  walker[it] = (2.0 / 3.0) * kin/(double)npart; // Temperature
  walker[ie] = 4.0 * v + kin + 8.0 * M_PI * rho * (pow(rcut, -9)/9. - pow(rcut, -3)/3.) * npart;  // Total energy;
  walker[ipr] = (p + ptail); //AGGIUNGERE PRESSIONE perché divido per npart se è intensiva? l'ho tolto
  
//  cout << " a questo punto la pressione è " << walker[ipr] << endl;
...
void Reset(int iblk) //Reset block averages
{
   ...
   if(iblk == 1) //perché solo iblk=1? guarda cosa sono globav
   {
   ...
   for(int i=0; i<n_props; ++i) //questo per tutti i blk?
   {
     blk_av[i] = 0;
   }
   blk_norm = 0;
...
void Accumulate(void) //Update block averages
{
 ...
 //    cout << "adesso invece " << blk_av[i] << endl;  //QUESTO PUNTO PRESSIONE È GIÀ 0
   }
   blk_norm = blk_norm + 1.0; //cos'è?
}
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    cout << "Acceptance rate " << accepted/attempted << endl << endl; //cos'é?
    
  //  Epot.open("S_epot.dat",ios::app);
 //   Ekin.open("S_ekin.dat",ios::app);
 //   Temp.open("S_temp.dat",ios::app);
 //   Etot.open("S_etot.dat",ios::app);
 //   Pres.open("S_pres.dat",ios::app);
    Gdr.open("S_gdr_MD.dat", ios::app);
   ...
   stima_kin = blk_av[ik]/blk_norm/(double)npart; //Kinetic energy
    ...
    stima_pres = (blk_av[ipr]/blk_norm)/(3.*vol) + rho * temp; //Pressure: perché non walker?
    ...
    for(int i=0; i<dimg; i++) { 
      ...
      //  cout << "funziona?" << endl;
    	double r = i * (min_dist)/dimg;
    	double dr = (min_dist)/dimg;
    	
    	g_norm = rho * npart * 4. * M_PI/3. *(pow(r+dr, 3) - pow(r,3));
    	
    	gdr_ave[i] = double(gdr_ave[i]/(double)blk_norm)/g_norm;
    	
 //   	cout << "norm " << g_norm << endl;
 //   	cout << "ave "<< gdr_ave[i] << endl;
...
//PRESSIONE  
    Pres << setw(wd) << iblk <<  setw(wd) << stima_pres << setw(wd) << glob_av[ipr]/(double)iblk << setw(wd) << err_press << endl;*/ 
    

    
    for(int i=0; i<dimg; i++) {
    
    	Gdr << gdr_ave[i] << " " ; // perché no endl?
