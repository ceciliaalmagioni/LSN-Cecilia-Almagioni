...
double pm1 = 0.1, pm2 = 0.1, pm3 = 0.1, pc = 0.55;
...
vector<vector<double>> read_from_file() {
    ...
    file.clear();  // Resetta lo stato del file
    file.seekg(0, ios::beg);  // Riporta il cursore all'inizio del file

    vector<vector<double>> dati(N_cities, vector<double>(2));

    for (int riga_idx = 0; riga_idx < N_cities; riga_idx++) {
        getline(file, riga);

        istringstream iss(riga);
        string valore;

        // Ignora le prime due colonne (stringhe)
        for (int colonna = 0; colonna < 2; colonna++) {
            iss >> valore;
        }

        // Leggi i valori delle ultime due colonne come double
        for (int colonna = 0; colonna < 2; colonna++) {
            double numero;
            iss >> numero;
            dati[riga_idx][colonna] = numero;
        }
    }
...
//	cout << rank << endl;
	...
	xy_city = read_from_file();
	if(rank==0) {for (int i=0 ; i<N_cities; i++) {
	
		cout << "città " <<i << " " << xy_city[i][0] << " " << xy_city[i][1] << endl;
	}}
