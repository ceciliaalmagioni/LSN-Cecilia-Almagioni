...
{ 
  ...
  int nconf = 1;
  for(int iblk=1; iblk <= nblk; iblk++) //Simulation su num blocchi
  {
    Reset(iblk);   //Reset block averages
    for(int istep=1; istep <= nstep; istep++) // su num step per blocco
    {
      Move();//attiva mossa MD sia MC --> MD microcanonico, MC canonico (T fissata)
      Measure();
      Accumulate(); //Update block averages accumulo val istantanei nel blocco
      if(istep%10 == 0){ //ogni 10 stem/100 etc scrivo vondig particelle e incremento
//        ConfXYZ(nconf);//Write actual configuration in XYZ format //Commented to avoid "filesystem full"! serve a stampare config dopo config le coordinate delle particelle (vedo come evolvono)
        nconf += 1;
      }
    }
    Averages(iblk);   //Print results for current block calcolo val di aspettt di blocco. all'ultimi i calcola valori finali e stampa
  }
...
//Read input informations
  ...
  ReadInput >> temp; //per MC
  beta = 1.0/temp; //unita LJ
...
//Prepare arrays for measurements
  ...
  ik = 2; //Kinetic energy
  ie = 3; //Total energy
  n_props = 4; //Number of observables. introdurre ip=4 per pressione e nprops=5
...
//Read initial configuration
  ...
  else 
  {
    ...
    {
      vx[i] = rnd.Gauss(0.,sqrt(temp));//distrib MaxwBoltz
    ...
    for (int idim=0; idim<3; ++idim) sumv[idim] /= (double)npart; //vtot per particella che sottraggo a tutte le tre part -->mom lin tot diverso da 0 (trasla e si aggiunge a stima temp) qui elimino
    double sumv2 = 0.0, fs;
    for (int i=0; i<npart; ++i)
    {
      vx[i] = vx[i] - sumv[0];
