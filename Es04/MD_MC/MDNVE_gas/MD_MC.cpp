...
//Print initial values for measured properties DOVE HA DICHIARATO WALKER, CHE È
...
// distance i-j in pbc = Minimum Image Convention
 ...
 /*   for (int i=0; i<npart; ++i){
    	...
    	for (int j=i+1; j<npart+1; j++) {// ciclo interno j=i+1...N
 ...
 //       cout << pr << endl;
  ...
  walker[ipr] = p; //AGGIUNGERE PRESSIONE perché divido per npart?
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=15;
    ...
    stima_pres = blk_av[ipr]/blk_norm/(3.*vol) + rho * temp; //Pressure
...
//Total energy
    Etot << setw(wd) << iblk <<  setw(wd)<< fixed << setprecision(12) << stima_etot << setw(wd) << glob_av[ie]/(double)iblk << setw(wd) << err_etot << endl;
//Temperature
